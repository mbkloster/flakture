...
// ==========================================================================
export const distanceCost = (ruleset, ctfGameState, moves, pieceCodex) => {
    ...
    BothSides.forEach(side => {
        ...
        Object.entries(pieces).forEach(orderNumberAndMove => {
            ...
            let lastDestination = ctfGameState.pieces[pieceIndex];
            let totalPieceDistance = 0;
            move.destinations.forEach(destination => {
                const nextDestination = destination;
                totalPieceDistance += Math.sqrt(Math.pow(nextDestination.y - lastDestination.y, 2) +
...
// ==========================================================================
export const stopGameMovement = (ruleset, ctfGameState, movement, finishedTurn) => {
    const newGameState = {
        ...ctfGameState,
    ...
    const distanceBonus = ctfGameState.decidingTurnNumber === 1
        ...
        : { left: 0, right: 0 };
    newGameState.distance.left = Math.min(ruleset.DISTANCE_MAX, newGameState.distance.left - movement.distanceTraveled.left + distanceBonus.left + ruleset.DISTANCE_PER_TURN);
    newGameState.distance.right = Math.min(ruleset.DISTANCE_MAX, newGameState.distance.right - movement.distanceTraveled.right + distanceBonus.right + ruleset.DISTANCE_PER_TURN);
    Object.entries(movement.pieces).forEach(indexAndPiece => {
...
//# sourceMappingURL=ctf-state-transition.js.map