...
// ==========================================================================
export const sideSubmitButton = (flakture, side) => {
...
// ==========================================================================
export const deriveDistanceFillProps = (flakture) => {
    ...
    BothSides.forEach(side => {
        ...
        result[side].fillAfterDistanceAndRefresh = 100 * Math.min(1, (flakture.gameState.distance[side] - sideCost + ruleset.DISTANCE_PER_TURN + distBonus) / ruleset.DISTANCE_MAX);
    });
    return result;
};
// ==========================================================================
export const clearRenderedDestinations = (flakture) => {
    flakture.ensureElemRemoved("dist-line");
};
// ==========================================================================
export const renderControlBar = (flakture) => {
    ...
    if (flakture.selectedPieceIndex !== null) {
        ...
        if (turn.moves[selectedPiece.side]?.pieces && turn.moves[selectedPiece.side].pieces[orderNumber]) {
            ...
            flakture.elem(`speed-radio-${turn.moves[selectedPiece.side].pieces[orderNumber].speed}`).click();
        }
        else {
            flakture.elem("control-bar-speed").classList.add("is-hidden");
            flakture.elem("control-bar-adjust").classList.add("is-hidden");
...
// ==========================================================================
export const renderDistanceFill = (flakture, distFillProps, side) => {
    flakture.elem(`control-bar-distance-box-fill-${side}`).setAttribute("style", `width: ${distFillProps[side].currentFill}%;`);
};
// ==========================================================================
...
// ==========================================================================
export const renderInitialControlBar = (flakture) => {
    ...
    const distFillProps = deriveDistanceFillProps(flakture);
    const createTopRowSideDetails = (side, reversed) => {
        let submitContents;
    ...
    const speedRadioAndLabel = (speedInt, icon) => {
    ...
    const nameElement = (side) => {
        ...
        if (flakture.controllingSides.includes(side)) {
            ...
            setupSelectedSideClick(flakture, button, side);
            return button;
        }
