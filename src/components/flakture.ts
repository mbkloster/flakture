...
import {
    ...
    CtfGameMovement,
    CtfGameProperties,
    CtfGameState, Formation, Line, Piece,
    PieceCodex,
    Ruleset,
    Side, SideMove,
    Turn
...
import {
    ...
    renderHoveredPiece,
    renderInitialBoard,
    renderMovedPieces, renderRedeployedPieces, renderRedeployLine, renderRedeployPoint,
...
export default class Flakture extends ApplicationComponent {
    conflictPoints: {x: number, y: number, showForSide: Side}[]
    controllingSides: Side[]
    ...
    redeploying: boolean
    redeployedPieces: {side: Side, order: number}[]
    redeployLines: Line[]
    redeployPoint: Coord
    renderRatio: number
    ruleset: Ruleset
            ...
            if (this.timeTicker >= ruleset.TIME_SLICE_S) {
                if (this.gameMovement) {
                    ...
                    if (firstImportantCollision) {
                        ...
                        const inlineTextAnimation = new InlineText(
                            this, resultText, textColor, outlineColor,
                            (collision.left.x + collision.right.x) / 2, (collision.left.y + collision.right.y) / 2,
                            0.1, 1, 0.4
                        );
    ...
    // ==========================================================================
    appendSelectedPieceDestination(coords: {x: number, y: number}) {
        ...
        if (existingSpeed !== undefined) {
            (this.elem(`speed-radio-${ existingSpeed }`) as HTMLElement).click();
        } else {
            (this.elem(`speed-radio-0`) as HTMLElement).click();
        }
        ...
        if (cost[selectedPiece.side] > this.gameState.distance[selectedPiece.side]) {
            this.addAnimation(
                new Notification(this, "Can't go that far!",
    ...
    finishRedeploy() {
        ...
        let firstDeadPiece: Piece = this.gameState.pieces[0];
        let order: number = 0;
    ...
    setRedeployedPieces() {
        ...
        Object.entries(this.currentTurn().moves).forEach(sideAndMove => {
            ...
            Object.entries(move!.redeployments).forEach(orderAndRedeployment => {
                ...
                this.redeployedPieces.push(
                    {side: side as Side, order: parseInt(order)}
                )
            });
        });
    }
    ...
    // ==========================================================================
    setRedeploying(redeploying: boolean) {
        this.redeploying = redeploying;
        if (redeploying) {
            this.redeployLines = redeployLines(this.gameProperties, this.gameState,
                this.currentTurn(), this.selectedSide!);
    ...
    // ==========================================================================
    setTurnMove(side: Side, move: SideMove, submitted: boolean) {
        const turn = this.currentTurn();
    ...
    // ==========================================================================
    updateHoveredPiece(hoveredPieceIndex: null | number) {
    ...
    // ==========================================================================
    updateSelectedPiece(selectedPieceIndex: null | number) {
